...
func _ready() -> void:
	...
	$Panel/ItemList.set_max_columns(3)
	...
	if (event is InputEventMouseButton):
		if (!isAwaitingSplit):
			if (event.is_action_pressed("mouse_leftbtn")):
				...
				initial_mousePos = get_global_mouse_position()
...
func _on_ItemList_item_rmb_selected(index:int, atpos:Vector2) -> void:
	...
	if (int(itemData["id"])) < 1: 
		print("broked")
		return
	var strItemInfo:String = ""

	#$Panel/WindowDialog_ItemMenu.set_position(get_global_mouse_position())
...
func begin_drag_item(index:int) -> void:
	...
	isDraggingItem = true
	mouseButtonReleased = false
	$Panel/Sprite_DraggedItem.global_translate(get_global_mouse_position())
...
func _on_SplitItemWindow_Button_Close_pressed() -> void:
	$Panel/WindowDialog_SplitItemWindow.hide()
